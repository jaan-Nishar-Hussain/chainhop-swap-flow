  ...
  return (
    <div className="relative">
      <button
        ...
        className="bg-gray-600 border border-gray-500 rounded-lg px-4 py-2 flex items-center space-x-2 hover:bg-gray-500 transition-all duration-200"
      >
        <span className="text-white font-medium">{token.symbol}</span>
        <ChevronDown className={`w-4 h-4 text-gray-300 transition-transform ${showTokens ? 'rotate-180' : ''}`} />
      </button>

      {showTokens && (
        <div className="absolute top-full right-0 mt-2 bg-gray-700 border border-gray-600 rounded-lg overflow-hidden z-50 min-w-48 shadow-lg">
          {tokens.map((tokenOption) => (
            <button
              key={tokenOption.symbol}
              onClick={() => handleTokenSelect(tokenOption)}
              className="w-full px-4 py-3 text-left hover:bg-gray-600 transition-all duration-200"
            >
